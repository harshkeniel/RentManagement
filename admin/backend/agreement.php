<?php
    ...
    if(isset($_POST['id'])){
        if(!empty($fdate) && !empty($tdate)){
            ...
            if($_FILES['agreement']['name'] != null){
                $fileName = $_FILES['agreement']['name'];
                $tmpName = $_FILES['agreement']['tmp_name'];
                $fileSplit = explode(".", $fileName);
                $fileExt = end($fileSplit);
                if($fileExt == "pdf"){
                    $time = $tdate."-".time();
                    $newFileName = $time."-agreement.pdf";
                    // $newFileName = str_replace(" ", "-", $newFileName);
                    if(move_uploaded_file($tmpName, "../../files/".$id."/agreement/".$newFileName)){
                        $sql = mysqli_query($conn, "SELECT * FROM agreement WHERE user_id = $id");
                        if(mysqli_num_rows($sql)==0){
                            $sql1 = mysqli_query($conn, "INSERT INTO agreement (user_id, fdate, tdate, file) VALUES ({$id}, '{$fdate}', '{$tdate}', '{$newFileName}')");
                            if($sql1){
                                echo "success";
                            }
                            else{
